        ...
        <div className="col-lg-12">
          ...
          <nav aria-label="Page navigation example">
            <ul className="pagination">
              <li className="page-item"><button disabled={currentPage===0} onClick={prev} className="page-link" >Previous</button></li>
              {
                [...Array(data.length / perPage)].map((page, index) => {
                  return (
                    <li key={index} className="page-item">
                      <button className={`page-link ${currentPage === index? 'active' : ""}`} onClick={() => pagenate(index)}>
                        {index + 1}
                      </button>
                    </li>
                  )
              ...
              <li className="page-item"><button disabled={currentPage === (data.length / perPage) -1} onClick={next} className="page-link" >Next</button></li>
            </ul>
            <select onChange={(e)=>{setPerPage(e.target.value)}} className="form-control" style={{width:75}}>
